...
def create_dictionary(fname, silent=True):
    global total_word_count, unique_word_count
    total_word_count = 0
    unique_word_count = 0
...
def init(corpus_path):
    import os
    import cPickle as pk

    global dictionary, longest_word_length, total_word_count, unique_word_count
    print "Please wait..."
    start_time = time.time()
    dict_path = corpus_path + '.bin'
    if os.path.isfile(dict_path):
        [longest_word_length, total_word_count, unique_word_count, dictionary] = pk.load(open(dict_path, 'r'))
    else:
        create_dictionary(corpus_path, False)
        pk.dump([longest_word_length, total_word_count, unique_word_count, dictionary], open(dict_path, 'w'))
